...
Request.prototype.attach = function(field, file, option){
  var opt = option || {};
  if ('string' == typeof option) opt = {filename: option};
  if ('string' == typeof file) {
    if (!opt.filename) opt.filename = file;
    debug('creating `fs.ReadStream` instance for file: %s', file);
    file = fs.createReadStream(file);
  } else if (!opt.filename && file.path) {
    opt.filename = file.path;
  }
  this._getFormData().append(field, file, opt);
  return this;
};
